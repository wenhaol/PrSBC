%Initializing multi-robot system with probabilistic safety barrier
...
si_barrier_certificate = create_si_pr_barrier_certificate_centralized('SafetyRadius', 2*SafetyRadius, 'Confidence', confidence_level);
...
record_video_flag = false; % flag for video recording
if record_video_flag
    writerObj = VideoWriter('PrSBC_centralized.mp4', 'MPEG-4');
    open(writerObj);
...
while timer_count< timer_to_stop  %(~init_checker(x(1:2,:), goal_condition)) %
    ...
    x_observe = [x(1:2,1:N)+pos_error; x(3,1:N)]; % put noise over ground-truth position to get noisy measurements.
...
min_bot_dist_hist_prsbc = min_bot_dist_hist; 
min_bot_dist_observe_hist_prsbc = min_bot_dist_observe_hist;
min_bot_safe_ratio_hist_prsbc = min_bot_safe_ratio_hist;
pertub_bot_hist_prsbc = pertub_bot_hist;
